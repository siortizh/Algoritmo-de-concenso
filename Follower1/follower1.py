...
class FollowerService(raft_pb2_grpc.RaftServiceServicer):
    ...
    def AppendEntries(self, request, context):
        ...
        print(f"Follower 1 ha recibido latido del líder en el término {self.term}")
    ...
    def RequestVote(self, request, context):
        if self.voted_for is None or request.term > self.term:
            ...
            print(f"Follower 1 ha votado por el candidato {request.candidate_id} en el término {self.term}")
            return raft_pb2.RequestVoteResponse(term=self.term, vote_granted=True)
        else:
            print(f"Follower 1 rechazó la solicitud de voto de {request.candidate_id} porque ya votó por {self.voted_for}")
    ...
    def start_election(self):
        ...
        self.voted_for = "follower 1"  # Reiniciar la votación
        ...
        for stub in self.peer_stubs:
            try:
                vote_request = raft_pb2.RequestVoteRequest(term=self.term, candidate_id="follower 1")
                response = stub.RequestVote(vote_request)
                if response.vote_granted:
                    self.votes_received += 1
                    print(f"Follower 1 recibió un voto. Total de votos: {self.votes_received}")
        ...
        if self.votes_received > len(self.peer_stubs) // 2:
            self.is_leader = True
            print(f"Follower 1 se ha convertido en el nuevo líder en el término {self.term}")
            self.send_heartbeat()
        else:
    ...
    def send_heartbeat(self):
        while self.is_leader:
            if self.leader_alive:  # Si detecta que el líder original volvió, detener latidos
            ...
            try:
                ...
                print("Enviando latido del corazón al follower 2.")
            except IndexError:
                print("Error: No se encontró el follower 2 en peer_stubs.")
            except grpc.RpcError as e:
...
def serve(peer_addresses, port):
    ...
    print(f"Servidor de Follower 1 en ejecución en el puerto {port}...")
    server.start()
    server.wait_for_termination()

if __name__ == "__main__":
    peer_addresses = ['172.31.47.254:50052', '172.31.33.34:50055'] #IP Leader e IP follower 2
    serve(peer_addresses=peer_addresses, port=50054)